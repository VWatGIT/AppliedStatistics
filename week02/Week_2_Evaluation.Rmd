...
setwd(r"(C:\AsAd\Python_Related\Applied_Statistics\AppliedStatistics\week02)")
...
plot_real <- ggplot(freq_real_df, aes(x=size, y=percent)) +
  geom_col(width = 0.2) +
  ...
  labs(
    ...
    y = "% of the size being drawn"
  ) +
  theme_minimal() 

ggsave("Real_Experiment.png", plot = plot_real, width = 6, height = 4)
...
set.seed(4)
sizes_sim <- sample(outcomes, size=10000, replace=TRUE, prob=P_uniform)
...
plot_sim <- ggplot(freq_sim_df, aes(x=size, y=percent))+
  geom_col(width = 0.2) +
  geom_point(size = 3, color = "red") +
  geom_smooth(
    method = "lm",
  ...
  labs(
    title = "Simulated Data from Drawn Pieces, N = 10000",
    x = "Size of drawn piece",
    y = "Percent of times the size was drawn"
  ) +
  theme_minimal() 
ggsave("Simulated_Experiment.png", plot = plot_sim, width = 6, height = 4)
```
...
plot_compare <- ggplot(freq_real_df, ) +
  ...
  theme_minimal()
ggsave("Observed_vs_Models.png", plot = plot_compare, width = 6, height = 4)
```
